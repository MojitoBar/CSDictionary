enum DB {
    static let csList = [
        CSItem(
            name: "Queue",
            shortDescription: """
            큐(Queue)는 데이터를 선형으로 저장하며, 선입선출(First In, First Out, FIFO) 원칙을 따릅니다.
            
            데이터는 한쪽 끝에서 추가되고, 반대쪽 끝에서 제거됩니다. 이로 인해 가장 먼저 추가된 요소가 가장 먼저 제거됩니다.
            
            큐의 주요 연산에는 'enqueue'로 데이터를 큐에 추가하고, 'dequeue'로 큐의 앞쪽에서 데이터를 제거하는 것이 있습니다.
            """,
            description: """
            큐는 컴퓨터 과학에서 다양한 상황에서 사용되는 중요한 자료구조 중 하나입니다. 예를 들어, 프린터의 인쇄 작업 관리, 운영 체제에서의 프로세스 스케줄링, 네트워크 요청의 처리 순서 결정 등에 큐가 활용됩니다.
